...
struct InitInfo {
    ...
    /// 去特征：默认值由 "LSPHooker_" 改为中性单字母（运行时生成类名是反 hook 扫描的主要指纹）。
    std::string_view generated_class_name = "Q";
    /// \brief The generated source name. Could be empty.
    /// 去特征：由 "LSP" 改为中性，且非空以避开 lsplant.cc 中 "lsplant" 兜底字面量。
    std::string_view generated_source_name = "S";
    /// \brief The generated field name. Must not be empty.
    /// 去特征：由 "hooker" 改为中性单字母。
    std::string_view generated_field_name = "f";
