...
bool disable_profile_saver() {
    ...
    process_profiling_info = NativeCore::elf_img.GetSymbolAddress(
            "_ZN3art12ProfileSaver20ProcessProfilingInfoEbPtb", false);
    if (!process_profiling_info) {
        const char *symbol;
        if (NativeCore::android_version < 26) {
        ...
        } else if (NativeCore::android_version < 31) {
        ...
        process_profiling_info = NativeCore::elf_img.GetSymbolAddress(symbol, false);
        ...
        if (!process_profiling_info && NativeCore::android_version >= 31) {
            symbol = "_ZN3art12ProfileSaver20ProcessProfilingInfoEbPt";
            process_profiling_info = NativeCore::elf_img.GetSymbolAddress(symbol, false);
        }
    }
